        ...
        window.addEventListener('scroll', () => {
            ...
            // Hero parallax
            const backdrop = document.querySelector('.water-backdrop');
            if (backdrop) {
                backdrop.style.transform = `translateY(${scrolled * 0.15}px)`;
            }
            
            // Treatment nodes parallax
            document.querySelectorAll('.treatment-node').forEach((node, index) => {
            ...
            // Flow streams effect
            document.querySelectorAll('.flow-stream').forEach((stream, index) => {
                const offset = scrolled * 0.12 * (index + 1);
                stream.style.transform = `translateX(${offset}px)`;
            });
        });

        // Enhanced card interactions
        document.querySelectorAll('.water-service-card, .application-category, .metric-card').forEach(card => {
        ...
        window.addEventListener('load', () => {
            ...
            // Initialize treatment node animations
            document.querySelectorAll('.treatment-node').forEach((node, index) => {
