        ...
        window.addEventListener('scroll', () => {
            ...
            // Hero parallax
            const backdrop = document.querySelector('.recycle-backdrop');
            if (backdrop) {
                backdrop.style.transform = `translateY(${scrolled * 0.15}px)`;
            }
            
            // Waste nodes parallax
            document.querySelectorAll('.waste-node').forEach((node, index) => {
            ...
            // Orbit rings effect
            document.querySelectorAll('.orbit-ring').forEach((ring, index) => {
                const offset = scrolled * 0.1 * (index + 1);
                ring.style.transform = `rotate(${offset}deg) scale(${1 + offset * 0.001})`;
            });
        });

        // Enhanced card interactions
        document.querySelectorAll('.waste-service-card, .waste-category, .metric-card').forEach(card => {
        ...
        const metricObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    const value = entry.target.querySelector('.metric-number');
        ...
        window.addEventListener('load', () => {
            ...
            // Initialize waste node animations
            document.querySelectorAll('.waste-node').forEach((node, index) => {
