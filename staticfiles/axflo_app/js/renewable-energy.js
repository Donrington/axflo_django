        ...
        window.addEventListener('scroll', () => {
            ...
            // Hero parallax
            const backdrop = document.querySelector('.solar-backdrop');
            if (backdrop) {
                backdrop.style.transform = `translateY(${scrolled * 0.15}px)`;
            }
            
            // Energy nodes parallax
            document.querySelectorAll('.energy-node').forEach((node, index) => {
            ...
            // Solar rays effect
            document.querySelectorAll('.solar-ray').forEach((ray, index) => {
                const offset = scrolled * 0.12 * (index + 1);
                ray.style.transform = `translateX(${offset}px) scaleX(${1 + offset * 0.001})`;
            });
        });

        // Enhanced card interactions
        document.querySelectorAll('.renewable-service-card, .energy-application, .metric-card').forEach(card => {
        ...
        window.addEventListener('load', () => {
            ...
            // Initialize energy node animations
            document.querySelectorAll('.energy-node').forEach((node, index) => {
