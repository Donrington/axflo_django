        ...
        window.addEventListener('scroll', () => {
            ...
            // Hero parallax
            const backdrop = document.querySelector('.response-backdrop');
            if (backdrop) {
                backdrop.style.transform = `translateY(${scrolled * 0.15}px)`;
            }
            
            // Response nodes parallax
            document.querySelectorAll('.response-node').forEach((node, index) => {
            ...
            // Emergency waves effect
            document.querySelectorAll('.emergency-wave').forEach((wave, index) => {
                const offset = scrolled * 0.1 * (index + 1);
                wave.style.transform = `translateX(${offset}px) scaleY(${1 + offset * 0.001})`;
            });
        });

        // Enhanced card interactions
        document.querySelectorAll('.osr-service-card, .equipment-category, .metric-card').forEach(card => {
        ...
        window.addEventListener('load', () => {
            ...
            // Initialize response node animations
            document.querySelectorAll('.response-node').forEach((node, index) => {
